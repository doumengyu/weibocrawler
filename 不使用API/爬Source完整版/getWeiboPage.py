...
class getWeiboPage:
    ...
    def get_firstpage(self,url):
        ...
        feed=resultes[0].find('div',attrs={'class': "WB_feed_type SW_fun "})
        ...
        try:
            ...
            cur = conn.cursor()
            sourcestring = 'insert ignore into user_source (uid,source) values ('+'\''+self.uid+'\''+','+'\''+srn+'\''+')'
            cur.execute(sourcestring)
            conn.commit()
            cur.close()
            conn.close()
        except Exception as e:
