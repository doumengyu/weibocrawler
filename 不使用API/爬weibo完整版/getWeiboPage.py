...
class getWeiboPage:
    ...
    def get_msg(self,uid):
        ...
        i = 2
        while i<getWeiboPage.pageNum+1 and self.isRunning:
    ...
    def get_firstpage(self,url):
        if self.isRunning:
            ...
            for feed in feedlist:
                ...
                ct=a[0]['date']
                srn=a[1].text
                ct_float=float(ct)/1000
                
                text=feed.findAll('div',attrs={'class': "WB_text"})
                WB_text=text[0].text
                
                if ct_float < 1346428800.0:
                    self.isRunning = 0
                    
                    return
                
                cur = conn.cursor()
                weibostring = 'insert ignore into user_weibo (uid,mid,createtime,source,text) values ('+'\''+self.uid+'\''+','+'\''+mid+'\''+','+'\''+ct+'\''+','+'\''+srn+'\''+','+'\''+WB_text+'\''+')'
                cur.execute(weibostring)
                cur.close()
            conn.commit()
            conn.close() 
